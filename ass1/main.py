...
class Trainer(object):
    ...
    def vae_loss(self, recon_x, x, mu, logvar):
        ...
        return recon_loss + 0.000000001 * kldivergence
    ...
    def train(self):
        ...
        for epoch in tqdm.tqdm(range(self.epochs + 1)):
            ...
            num_batches = 0
            if epoch > 495:
                torch.save(self.net.state_dict(), "_".join([f'./datasets/toothbrush/model', str(epoch), '.pth']))
            for batch_idx, samples in enumerate(self.dataloader):
                x_train, y_train = samples
                x_train, y_train = x_train.to(device), y_train.to(device)
                g, latent_mu, latent_logvar = self.net(x_train)
                loss = self.vae_loss(g, x_train,latent_mu, latent_logvar)
            ...
            if (epoch+1)%10 == 0:
                ...
                print('Epoch [%d / %d] average reconstruction error :  %f' % (epoch+1, self.epochs + 1,  train_loss_avg))
...
class Tester(object):
    def __init__(self, batch_size):
        ...
        # Load of pretrained_weight file
        weight_PATH = f'./model_capsule_500.pth' #Change this path

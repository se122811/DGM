...
def WGAN_tester(opt):
    ...
    def load_model_generator(net, epoch, opt):
        ...
        model_name = 'deepfillv2_WGAN_G_epoch1_batchsize32.pth'
    ...
    for batch_idx, (img, mask) in enumerate(dataloader):
        ...
        masked_img = img * (1 - mask)# * 0.7 + img * mask * 0.3
        mask = torch.cat((mask, mask, mask), 1)
        # img_list = [img, second_out_wholeimg, third_out_wholeimg]
        # name_list = ['masked_img', 'second_out', 'third_out']
        img_list = [second_out_wholeimg, third_out_wholeimg]
        name_list = ['second_out', 'third_out']
        utils.save_sample_png(sample_folder = results_path, sample_name = '%d' % (batch_idx + 1), img_list = img_list, name_list = name_list, pixel_max_cnt = 255, order = batch_idx)

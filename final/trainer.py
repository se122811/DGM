...
def WGAN_trainer(opt):
    ...
    discriminator2 = utils.create_discriminator(opt)
    ...
    optimizer_d = torch.optim.Adam(discriminator2.parameters(), lr = opt.lr_d, betas = (opt.b1, opt.b2), weight_decay = opt.weight_decay)
    optimizer_d2 = torch.optim.Adam(discriminator2.parameters(), lr = opt.lr_d, betas = (opt.b1, opt.b2), weight_decay = opt.weight_decay)
    ...
    load_model(generator, 1, opt, type='G')
    load_model(discriminator, 1, opt, type='D')
    ...
    for epoch in range(opt.resume_epoch, opt.epochs):
        for batch_idx, (img, height, width) in enumerate(dataloader):
            ...
            # Generator output
            first_out, second_out, third_out = generator(img, mask)

            # forward propagation
            first_out_wholeimg = img * (1 - mask) + first_out * mask        # in range [0, 1]
            second_out_wholeimg = img * (1 - mask) + second_out * mask      # in range [0, 1]
            third_out_wholeimg = img * (1 - mask) + third_out * mask      # in range [0, 1]

            # Fake samples
            fake_scalar = discriminator(second_out_wholeimg.detach(), mask)
            fake_scalar2 = discriminator2(third_out_wholeimg.detach(), mask)
            # True samples
            true_scalar = discriminator(img, mask)
            true_scalar2 = discriminator2(img, mask)
            
            # Loss and optimize
            loss_fake = -torch.mean(torch.min(zero, -valid-fake_scalar))
            loss_true = -torch.mean(torch.min(zero, -valid+true_scalar))
            loss_fake2 = -torch.mean(torch.min(zero, -valid-fake_scalar2))
            loss_true2 = -torch.mean(torch.min(zero, -valid+true_scalar2))
            # Overall Loss and optimize
            # loss_D = 0.5 * (loss_fake + loss_true)
            loss_D = 0.5 * (loss_fake2 + loss_true2)
            loss_D.backward()
            # loss_D2.backward()
            optimizer_d.step()
            # optimizer_d2.step()
            ...
            third_L1Loss = (third_out - img).abs().mean()
            
            # GAN Loss
            fake_scalar = discriminator(second_out_wholeimg, mask)
            fake_scalar2 = discriminator2(third_out_wholeimg, mask)
            GAN_Loss = -torch.mean(fake_scalar) + -torch.mean(fake_scalar2)

            # Get the deep semantic feature maps, and compute Perceptual Loss
            img_featuremaps = perceptualnet(img)                          # feature maps
            second_out_featuremaps = perceptualnet(second_out)
            third_out_featuremaps = perceptualnet(third_out)
            second_PerceptualLoss = L1Loss(second_out_featuremaps, img_featuremaps)
            third_PerceptualLoss = L1Loss(third_out_featuremaps, img_featuremaps)

            # Compute losses
            loss = opt.lambda_l1 * first_L1Loss + opt.lambda_l1 * second_L1Loss * 0.1 + opt.lambda_l1 * third_L1Loss + \
                   opt.lambda_perceptual * second_PerceptualLoss * 0.1 + \
                   opt.lambda_perceptual * third_PerceptualLoss + opt.lambda_gan * GAN_Loss
            ...
            # Print log
            print("\r[Epoch %d/%d] [Batch %d/%d] [first Mask L1 Loss: %.5f] [second Mask L1 Loss: %.5f] [third Mask L1 Loss: %.5f]" %
                ((epoch + 1), opt.epochs, batch_idx, len(dataloader), first_L1Loss.item(), second_L1Loss.item(), third_L1Loss.item()))
            ...
            if (batch_idx + 1) % 40 == 0:
                ...
                name_list = ['gt', 'mask', 'masked_img', 'first_out', 'second_out', 'third_out']
        ...
        # adjust_learning_rate(opt.lr_d, optimizer_d2, (epoch + 1), opt)

        # Save the model
        save_model_generator(generator, (epoch + 1), opt)
        save_model_discriminator(discriminator, (epoch + 1), opt)
        # save_model_discriminator(discriminator2, (epoch + 1), opt)
